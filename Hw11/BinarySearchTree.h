...
void BinarySearchTree<ItemType>::levelorder(void visit(BinaryNode<ItemType>&), BinaryNode<ItemType>* treePtr) const
{
	...
	// level 越高的越晚 visit ，同一層 level 的節點 (sibling nodes) 則由左至右 visit。
	// ==============================================
                                                            ...
                                                            BinaryNode<ItemType>* newNodePtr)
{
	...
	// replace the code with yours
	// 使得重複的 word 加入 search tree 時，不會產生新的節點，而是記錄該 word 已被加入幾次。
	// ====================finish========================
                                     ...
                                     bool& success)
{
	...
	if ( subTreePtr->getItem() == target ) {
		// 除非 count 是 1 ，否則不移除該節點，而僅是減少 count 記錄的次數。
                                            ...
                                            ItemType& inorderPredecessor, int& newWordCount)
{
	...
	{
		tempPtr = removeRightmostNode(nodePtr->getRightChildPtr(), inorderPredecessor, newWordCount);
		nodePtr->setRightChildPtr(tempPtr);
		return nodePtr;
	}
}
