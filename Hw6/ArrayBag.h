...
bool ArrayBag<ItemType>::addFirst(const ItemType& newEntry)
{
	/*
		...
		3.  else
			return false;
	*/
	if(itemCount < maxItems){ // 是否還有空間插入
		items[itemCount] = items[0]; // 第一個放最後一個
		items[0] = newEntry; // 新的放第一個
		itemCount++;
...
bool ArrayBag<ItemType>::insertFirst(const ItemType& newEntry)
{
	/*
		...
		2.  如果有空間
			...
			所以要先將
			items[1] ~ items[itemCount] 都往後移一個
			items[0] = 新的;
			itemCount++;
			return true;
		3.  else
...
bool ArrayBag<ItemType>::removeFirst()
{
	...
	*/
	if(!isEmpty()){
		// itemCount 為 6 array 的 index 為 5
